...
/* ----------------------------- building visits (check in/out) ----------------------------- */

// Returns the list of buildings (id + name) for the check-in picker and QR page.
...
// The worker's currently-open visit (checked in, not yet checked out), if any.
export async function fetchOpenVisit(workerId) {
  const { data, error } = await supabase
    .from("building_visits")
    .select("id, building_id, request_id, check_in_at, buildings ( name ), maintenance_requests ( reference_code )")
  ...
  return {
    ...
    requestCode: data.maintenance_requests?.reference_code ?? null,
    checkInAt: data.check_in_at,
  };
}

export async function checkIn({ workerId, buildingId, requestId }) {
  const { error } = await supabase.from("building_visits").insert({
    worker_id: workerId,
...
export async function checkOut({ visitId, note }) {
  const { error } = await supabase
    .from("building_visits")
    .update({ check_out_at: new Date().toISOString(), work_note: note || null })
    .eq("id", visitId);
  if (error) throw error;
}

// All visits, newest first — for the admin attendance page.
export async function fetchAllVisits() {
  const { data, error } = await supabase
    .from("building_visits")
    .select("id, check_in_at, check_out_at, work_note, worker:worker_id ( full_name ), buildings ( name ), maintenance_requests ( reference_code )")
    .order("check_in_at", { ascending: false })
    .limit(200);
  if (error) throw error;
  return (data || []).map((v) => ({
    id: v.id,
    worker: v.worker?.full_name ?? "-",
    building: v.buildings?.name ?? "-",
    requestCode: v.maintenance_requests?.reference_code ?? null,
