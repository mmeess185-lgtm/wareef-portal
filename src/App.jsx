...
function Sidebar({ role, t, view, setView, mobileOpen, setMobileOpen, lang }) {
    ...
    <>
      ...
      >
        ...
        <div className="p-3 border-t border-slate-200 dark:border-slate-800">
          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
            <LogOut size={18} />
            {t.logout}
          </button>
        </div>
...
function Topbar({ t, lang, setLang, dark, setDark, role, setRole, setMobileOpen, title, notifCount }) {
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        <div className="relative">
          <button
            ...
            className="hidden sm:flex items-center gap-2 text-xs font-semibold px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700"
          >
            {t[role]}
            <ChevronRight size={12} className="rotate-90" />
          </button>
          {roleMenu && (
            <div className="absolute right-0 mt-2 w-44 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg overflow-hidden">
              <p className="text-[10px] font-semibold text-slate-400 uppercase px-3 pt-2.5 pb-1">{t.switchRole}</p>
              {["tenant", "coordinator", "technician"].map((r) => (
                <button
                  key={r}
                  onClick={() => { setRole(r); setRoleMenu(false); }}
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-800 ${role === r ? "text-blue-600 font-semibold" : "text-slate-600 dark:text-slate-300"}`}
                >
                  {t[r]}
                </button>
              ))}
        ...
        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-600 to-sky-500 text-white flex items-center justify-center text-xs font-bold ml-1">
          {role === "tenant" ? "AF" : role === "coordinator" ? "CO" : "KS"}
        </div>
      </div>
    </header>
  );
...
function TenantProfile({ t, lang }) {
  const myReqs = initialRequests.filter((r) => r.unit === CURRENT_TENANT.unit);
  return (
...
const CURRENT_TECH = TECHNICIANS[0];
...
export default function App() {
  ...
  const openRequest = (r) => setSelected(r);
  const liveSelected = selected ? requests.find((r) => r.id === selected.id) : null;

  const submitNewRequest = (form) => {
    const id = "REQ-" + (1043 + requests.filter((r) => r.unit === CURRENT_TENANT.unit).length + Math.floor(Math.random() * 50));
    const newReq = {
      id, unit: CURRENT_TENANT.unit, building: CURRENT_TENANT.building, tenant: CURRENT_TENANT.name,
      category: form.category, priority: form.priority, status: "new", description: form.description,
      createdAt: new Date().toISOString(), preferredTime: form.preferredTime, technician: null,
      notes: [], chat: [], photos: form.photos, rating: null,
    };
    setRequests((rs) => [newReq, ...rs]);
    setView("requests");
  };

  const changeRole = (r) => { setRole(r); setView("dashboard"); setSelected(null); };
  ...
  if (liveSelected) {
    ...
    else body = <TenantProfile t={t} lang={lang} />;
  ...
  return (
    <div dir={lang === "ar" ? "rtl" : "ltr"} className={dark ? "dark" : ""}>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans flex">
        <Sidebar role={role} t={t} view={view} setView={(v) => { setView(v); setSelected(null); }} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} lang={lang} />
        <div className="flex-1 min-w-0 flex flex-col">
          <Topbar t={t} lang={lang} setLang={setLang} dark={dark} setDark={setDark} role={role} setRole={changeRole} setMobileOpen={setMobileOpen} title={title} notifCount={requests.filter(r=>r.priority==="emergency"&&r.status!=="closed").length} />
