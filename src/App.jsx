...
import {
  CURRENT_TENANT_ID, CURRENT_TECHNICIAN_ID,
...
export default function App() {
  ...
  // Load technicians + tenant profile once on first mount (shared across roles).
  React.useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        CURRENT_TECH = TECHNICIANS.find((x) => x.id === CURRENT_TECHNICIAN_ID) || TECHNICIANS[0] || CURRENT_TECH;

        const tenantCtx = await getTenantContext(CURRENT_TENANT_ID);
        if (cancelled) return;
      ...
      try {
        ...
        if (role === "tenant") {
          if (!CURRENT_TENANT._unitId) {
            const ctx = await getTenantContext(CURRENT_TENANT_ID);
          ...
          data = await fetchRequestsForTechnician(CURRENT_TECHNICIAN_ID);
        }
  ...
  const updateRequest = async (id, patch) => {
    ...
    try {
      if (patch.status && patch.status !== req.status) {
        await updateRequestStatus(req.dbId, patch.status, role === "tenant" ? CURRENT_TENANT_ID : CURRENT_TECHNICIAN_ID, req.status);
      }
      ...
      if (patch.notes && patch.notes.length > req.notes.length) {
        ...
        await addNote(req.dbId, role === "technician" ? CURRENT_TECHNICIAN_ID : CURRENT_TENANT_ID, newest.text);
      }
      if (patch.chat && patch.chat.length > req.chat.length) {
        const newest = patch.chat[patch.chat.length - 1];
        await sendChatMessage(req.dbId, role === "tenant" ? CURRENT_TENANT_ID : CURRENT_TECHNICIAN_ID, newest.text);
      }
