...
import {
  ...
  fetchBuildings, fetchOpenVisit, checkIn, checkOut, fetchAllVisits,
} from "./lib/api";
import { signInWithPassword, signUpWithPassword, sendPasswordReset, updatePassword, getSession, onAuthStateChange, signOut, fetchMyProfile } from "./lib/auth";
...
const T = {
  en: {
    ...
    checkInOut: "Check In / Out", attendance: "Attendance", checkIn: "Check In", checkOut: "Check Out",
    onSiteNow: "You're checked in", selectBuilding: "Select building", selectTask: "Which task? (optional)",
    workNote: "What did you do?", duration: "Duration", notCheckedIn: "You're not checked in anywhere.",
    buildingQR: "Building QR Code",
  },
  ar: {
    ...
    checkInOut: "تسجيل الدخول / الخروج", attendance: "الحضور", checkIn: "تسجيل دخول", checkOut: "تسجيل خروج",
    onSiteNow: "أنت مسجّل دخول", selectBuilding: "اختر المبنى", selectTask: "أي مهمة؟ (اختياري)",
    workNote: "ماذا أنجزت؟", duration: "المدة", notCheckedIn: "لست مسجّل دخول في أي مكان.",
    buildingQR: "رمز QR للمبنى",
  },
};
...
function NAV_ITEMS(role, t) {
  ...
  return [
    ...
    { id: "checkin", label: t.checkInOut, icon: MapPin },
    { id: "requests", label: t.requests, icon: ClipboardList },
    { id: "profile", label: t.profile, icon: User },
  ];
}
...
function LoginScreen() {
  ...
  const heading =
    mode === "signin" ? "Sign in to Wareef" : mode === "signup" ? "Create your password" : "Reset your password";
  const subtext =
    mode === "signin"
      ? "Use the email and password your coordinator set up for you."
      : mode === "signup"
      ? "First time here? Set a password for the email your coordinator already registered."
      ...
      <Card className="w-full max-w-sm p-7">
        ...
        {signedUp ? (
          <div className="text-center">
            ...
            <p className="font-semibold text-slate-900 mb-1">Account created</p>
            <p className="text-sm text-slate-500 mb-4">You can now sign in with your email and password.</p>
            <Button className="w-full" onClick={() => { setMode("signin"); setSignedUp(false); }}>Go to sign in</Button>
          </div>
        ) : resetSent ? (
          <div className="text-center">
            <div className="w-12 h-12 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto mb-4">
            ...
            <p className="text-sm text-slate-500 mb-4">We sent a password-reset link to <span className="font-medium">{email}</span>. Open it on this device to choose a new password.</p>
            <Button className="w-full" onClick={() => { setMode("signin"); setResetSent(false); }}>Back to sign in</Button>
          </div>
        ) : (
          <form onSubmit={submit}>
            <p className="font-semibold text-slate-900 text-center mb-1">{heading}</p>
            <p className="text-sm text-slate-500 text-center mb-5">{subtext}</p>
            <input
              type="email"
              required
              value={email}
            ...
            <Button type="submit" className="w-full mb-3" disabled={busy}>
              {busy ? "Please wait…" : mode === "signin" ? "Sign in" : mode === "signup" ? "Create password & sign up" : "Send reset link"}
            </Button>
            {mode === "signin" && (
              <button
                type="button"
                onClick={() => { setMode("forgot"); setError(""); }}
                className="w-full text-center text-xs text-slate-500 hover:text-blue-600 mb-2"
              >
            ...
            >
              {mode === "signin" ? "First time here? Set up your password" : mode === "signup" ? "Already have a password? Sign in" : "Back to sign in"}
            </button>
          </form>
        )}
      </Card>
...
function ResetPasswordScreen({ onDone }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err.message || "Couldn't update the password. Please try again.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-sm p-7">
        ...
        <form onSubmit={submit}>
          <p className="font-semibold text-slate-900 text-center mb-1">Choose a new password</p>
          <p className="text-sm text-slate-500 text-center mb-5">Enter a new password for your account.</p>
          <input
            type="password"
            required
            minLength={6}
...
function durationText(inAt, outAt) {
  ...
  const mins = Math.max(0, Math.round((end - new Date(inAt)) / 60000));
  const h = Math.floor(mins / 60);
  const m = mins % 60;
  return h > 0 ? `${h}h ${m}m` : `${m}m`;
}

function CheckInScreen({ profile, requests, lang, t }) {
  ...
  // If the QR link carried a specific building, lock to it (no dropdown).
  const qrBuilding = (() => {
    try { return new URLSearchParams(window.location.search).get("building") || ""; }
    catch { return ""; }
  })();
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [b, v] = await Promise.all([fetchBuildings(), fetchOpenVisit(profile.id)]);
      setBuildings(b);
      setOpenVisit(v);
      const preferred = qrBuilding && b.find((x) => x.id === qrBuilding) ? qrBuilding : (b[0]?.id || "");
      if (!buildingId) setBuildingId(preferred);
    } catch (e) { console.error(e); }
    finally { setLoading(false); }
  };

  React.useEffect(() => { load(); /* eslint-disable-next-line */ }, []);
  // live-updating duration while checked in
  React.useEffect(() => {
    if (!openVisit) return;
  ...
  const doCheckIn = async () => {
    ...
    setBusy(true);
    try {
      await checkIn({ workerId: profile.id, buildingId, requestId });
      await load();
    } catch (e) { alert("Couldn't check in. Please try again."); console.error(e); }
  ...
  const doCheckOut = async () => {
    setBusy(true);
    try {
      await checkOut({ visitId: openVisit.id, note });
      setNote("");
      await load();
    } catch (e) { alert("Couldn't check out. Please try again."); console.error(e); }
    finally { setBusy(false); }
  };
      ...
      {openVisit ? (
        <Card className="p-6">
          <div className="flex items-center gap-2 text-emerald-600 font-semibold mb-1">
            <CheckCircle2 size={18} /> {t.onSiteNow}
          </div>
          <p className="text-sm text-slate-500 mb-4">{openVisit.buildingName}{openVisit.requestCode ? ` · ${openVisit.requestCode}` : ""}</p>
          <div className="bg-slate-50 dark:bg-slate-800/60 rounded-xl p-4 mb-4 text-center">
            <p className="text-xs text-slate-400 mb-1">{t.duration}</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white">{durationText(openVisit.checkInAt)}</p>
            <p className="text-xs text-slate-400 mt-1">since {fmtDate(openVisit.checkInAt, lang)}</p>
          </div>
          <label className="text-xs text-slate-400 mb-1.5 block">{t.workNote}</label>
          <textarea
            value={note}
            onChange={(e) => setNote(e.target.value)}
            rows={3}
            placeholder="e.g. Replaced AC capacitor, tested cooling"
            className="w-full text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3.5 py-2.5 mb-4 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <Button variant="success" className="w-full" onClick={doCheckOut} disabled={busy}>
            <XCircle size={16} /> {t.checkOut}
          </Button>
        </Card>
      ) : (
        <Card className="p-6">
          <div className="flex items-center gap-2 text-slate-900 dark:text-white font-semibold mb-4">
            <MapPin size={18} /> {t.checkIn}
          </div>
          {qrBuilding && buildings.find((b) => b.id === qrBuilding) ? (
            <div className="mb-4">
              <label className="text-xs text-slate-400 mb-1.5 block">{t.selectBuilding}</label>
              <div className="text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3.5 py-2.5 font-medium text-slate-900 dark:text-white">
                {buildings.find((b) => b.id === qrBuilding)?.name}
              </div>
            </div>
          ) : (
            <>
          ...
          <label className="text-xs text-slate-400 mb-1.5 block">{t.selectTask}</label>
          <select
            value={requestId}
          ...
          >
            <option value="">—</option>
            {requests.filter((r) => !["closed"].includes(r.status)).map((r) => (
              <option key={r.dbId} value={r.dbId}>{r.id} · {catInfo(r.category)[lang]}</option>
            ))}
          </select>
          <Button className="w-full" onClick={doCheckIn} disabled={busy || !buildingId}>
            <MapPin size={16} /> {t.checkIn}
          </Button>
        </Card>
      )}
    </div>
...
function AttendancePage({ lang, t }) {
  ...
  if (loading) return <div className="text-center py-20 text-slate-400 text-sm">Loading…</div>;
  if (!visits.length) return <Card><EmptyState icon={Clock} title="No check-ins yet" sub="Worker visits will appear here once they scan and check in." /></Card>;
      ...
      {visits.map((v) => (
        <Card key={v.id} className="p-4">
          <div className="flex items-start justify-between gap-3 mb-2">
            <div>
              <p className="font-semibold text-slate-900 dark:text-white text-sm">{v.worker}</p>
              <p className="text-xs text-slate-400">{v.building}{v.requestCode ? ` · ${v.requestCode}` : ""}</p>
            </div>
            <Pill color={v.checkOutAt ? "#10B981" : "#F59E0B"} bg={v.checkOutAt ? "#ECFDF5" : "#FFFBEB"}>
              {v.checkOutAt ? durationText(v.checkInAt, v.checkOutAt) : "On site"}
            </Pill>
          </div>
          <div className="flex items-center gap-4 text-xs text-slate-500">
            <span>In: {fmtDate(v.checkInAt, lang)}</span>
            <span>Out: {v.checkOutAt ? fmtDate(v.checkOutAt, lang) : "—"}</span>
          </div>
          {v.note && <p className="text-sm text-slate-600 dark:text-slate-300 mt-2 pt-2 border-t border-slate-100 dark:border-slate-800">{v.note}</p>}
        </Card>
      ))}
    </div>
  );
...
// Printable per-building QR codes that open the check-in view for that building.
function BuildingQRButton({ t }) {
  const [items, setItems] = useState([]); // {id, name, dataUrl}

  React.useEffect(() => {
    (async () => {
      try {
        const bs = await fetchBuildings();
        const withQR = await Promise.all(bs.map(async (b) => {
          const url = `${window.location.origin}/?checkin=1&building=${b.id}`;
          const dataUrl = await QRCode.toDataURL(url, { width: 600, margin: 2 });
          return { id: b.id, name: b.name, dataUrl };
        }));
        setItems(withQR);
      } catch (e) { console.error(e); }
    })();
  ...
  const printQR = (item) => {
    ...
    w.document.write(`
      <html><head><title>${item.name} — Check-In QR</title>
      <style>body{font-family:Arial;text-align:center;padding:40px}h1{color:#2563eb;margin-bottom:4px}h2{margin-top:0}img{width:340px}</style>
      </head><body>
      <h1>Wareef Maintenance</h1>
      <h2>${item.name}</h2>
      <h3>Scan to Check In / Out</h3>
      <img src="${item.dataUrl}" />
      <p style="color:#64748b;max-width:400px;margin:16px auto">Workers: scan, sign in, then tap Check In when you arrive and Check Out when you leave.</p>
      </body></html>`);
    w.document.close();
    w.print();
  };
  ...
  return (
    <div className="mb-6">
      <p className="font-semibold text-slate-900 dark:text-white mb-3 flex items-center gap-2"><QrCode size={18} /> {t.buildingQR}</p>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((item) => (
          <Card key={item.id} className="p-5 text-center">
            <p className="font-semibold text-slate-900 dark:text-white text-sm mb-3">{item.name}</p>
            <img src={item.dataUrl} alt={`${item.name} QR`} className="w-40 h-40 mx-auto mb-4 rounded-lg border border-slate-200" />
            <Button variant="secondary" className="w-full" onClick={() => printQR(item)}><FileText size={15} /> Print QR</Button>
          </Card>
        ))}
      </div>
    </div>
...
const TITLE_MAP = {
  ...
  coordinator: { dashboard: "dashboard", requests: "requests", technicians: "technicians", attendance: "attendance", reports: "reports" },
  technician: { dashboard: "myJobs", checkin: "checkInOut", requests: "requests", profile: "profile" },
};
...
export default function App() {
  ...
  const [view, setView] = useState(() => {
    try { return new URLSearchParams(window.location.search).get("checkin") ? "checkin" : "dashboard"; }
  ...
  if (liveSelected) {
    ...
    else if (view === "attendance") body = <><BuildingQRButton t={t} /><AttendancePage lang={lang} t={t} /></>;
    ...
    else if (view === "checkin") body = <CheckInScreen profile={profile} requests={requests} lang={lang} t={t} />;
